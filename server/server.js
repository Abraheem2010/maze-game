...
// שמור את ה-DB בתוך תיקיית server (יותר יציב ב-Render)
...
// פינג לבדיקה מהירה (גם ל-Render health check אם תרצה)
...
app.post("/api/score", (req, res) => {
  ...
  db.get("SELECT time FROM records WHERE stage = ?", [stage], (err, row) => {
    ...
    // יש שיא: מעדכנים רק אם הזמן חדש קטן יותר
    if (time < row.time) {
      db.run(
...
if (fs.existsSync(indexHtml)) {
  ...
  // SPA fallback (React Router) — אבל לא ל-/api
  app.get("*", (req, res, next) => {
    if (req.path.startsWith("/api")) return next();
    res.sendFile(indexHtml);
  });
} else {
  // אם אין build — לפחות שהשורש יחזיר משהו ברור
  app.get("/", (req, res) => {
